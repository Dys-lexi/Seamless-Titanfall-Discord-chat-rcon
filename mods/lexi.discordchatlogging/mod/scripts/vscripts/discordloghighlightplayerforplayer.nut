...
void function actuallyhighlight( table<entity, array<entity> > entityhighlights, string highlightname, vector colour, int insideslot, float radius )
{
	...
	foreach ( player1real, value in entityhighlights )
	{
		...
		foreach ( entity player2real in value )
		{
			...
			if ( !IsValid( player2real ) )
			{
				continue
			}

			if ( !IsValid( player1real ) )
			{
				continue
			}
			...
			// /compilestring function:foreach(entity cPlayer in GetPlayerArray()){cPlayer.SetBossPlayer( discordlogmatchplayers("allu")[0]) ; Highlight_SetOwnedHighlight(cPlayer,"interact_object_los")}
			// servername:Aegisattritionihh
			highlight.paramVecs[ 0 ]
				= colour // <0.8,0.4,0.2>
